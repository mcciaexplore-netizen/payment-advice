...
import { notifySentBack, notifySubmissionRecommended } from "@/lib/email/notify";
...
export type AuthorityRecommendationState = AuthorityActionState & {
  serialNo: string;
...
export async function performAuthorityApproval(input: {
  advice: AuthorityRecommendationState;
  actor: string;
  ...
  await notifySubmissionRecommended(
    {
