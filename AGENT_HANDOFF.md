...
**Last updated:** 15 September 2026, by Codex (GST flag + optional payment remarks)

### Completed on feature branch — Arrears/TDS Payable Calculator + GST Settlement Flag (Codex, 2026-09-15; pending merge/deployment)
...
- The separate `gst_settlements` table was removed before deployment. A read-only production check confirmed that table does not exist because migration 0021 was never applied, and the isolated development version contained zero rows, so simplifying to direct `payment_advices` columns loses no real or test settlement history. Additive migration `0021_noisy_multiple_man.sql` contains the nullable calculator columns plus the three direct GST flag columns. **It has not been applied to production.** Historical `bill_passed_for` values are untouched.
...
- 🟢 **The 2026-09-15 local schema/code mismatch is isolated and no longer blocks `main`.** The complete calculator/GST work is committed and pushed on `gst-settlement-payable-calculator-wip`; `main` deliberately excludes it until migration 0021 is approved. Codex rebuilt a separate persistent local PostgreSQL database through `0021_noisy_multiple_man.sql` and used it for the feature's live tests. Do not point this feature branch at an unmigrated database and mistake the expected missing-column error for an application regression.
