...
**Last updated:** 21 September 2026, by Claude (picked up from Codex mid-session — usage limit — verified, live-tested, and extended the Gemini invoice auto-fill; still uncommitted)
...
- **Still entirely uncommitted** — no commits exist on `feature/gemini-invoice-autofill` (its tip still equals `main`); everything above, Codex's original work included, is sitting in the working tree pending the human's go-ahead to commit.
...
- Merged to `main` on 16 September 2026 together with the Finance Vendor Review Queue, after TypeScript, ESLint, the full Vitest suite, and a production build passed.

### Shipped — Arrears/TDS Payable Calculator + GST Settlement Flag (Codex, 2026-09-16)
...
- Regular NEFT with GST now has an independent post-verification **Yes/No GST settlement flag**. The UI shows the submitted GST amount and one Yes/No radio group only; it captures no settlement amount or remarks. Selecting Yes atomically stores `gst_settled = true`, the logged-in Finance user's name in `gst_settled_by`, and a timestamp in `gst_settled_at`, with an audit entry. It remains actionable after the main payable amount is fully settled. Zero-GST submissions and Advance remain excluded.
- The separate `gst_settlements` table was removed before deployment. A read-only production check confirmed that table did not exist and the isolated development version contained zero rows, so simplifying to direct `payment_advices` columns loses no real or test settlement history. The final additive migration is `0023_payable_gst.sql` (renumbered after migrations 0021/0022 landed). With explicit human approval, it initializes `payable_amount` from the already-saved `bill_passed_for` only for existing rows that already have payment entries; at approval time this matched exactly `MCCIA/2026-27/0001` (₹5,040 passed, ₹4,800 paid). It does not alter `bill_passed_for`, recorded payments, or serial numbers.
- Migration `0023_payable_gst` was applied successfully to production on 16 September 2026. Post-migration read-back confirmed all seven intended columns and exactly one compatibility row: `MCCIA/2026-27/0001` retained `bill_passed_for = 5040.00`, gained `payable_amount = 5040.00`, and retained its one ₹4,800 payment (`total_paid` and payment-entry sum both `4800.00`).
- Record a Payment remarks are now optional in both client and server validation. The existing `payment_entries.remarks NOT NULL` column is intentionally retained for backward schema compatibility; an omitted remark is persisted as the empty string and is not rendered as a blank history paragraph. Amount validation, payable cap, recorder attribution, and locking are unchanged.
- Isolated live test (persistent local PostgreSQL, no production mutation): Basic ₹10,000 + Arrears ₹20,000 + shared 10% returned and rendered Current TDS ₹1,000, Arrears TDS ₹2,000, Total TDS ₹3,000, Payable ₹7,000. The GST prompt rendered with submitted GST ₹1,800 and no amount/remarks form; selecting Yes returned success and DB read-back confirmed `true | Dev Finance | timestamp`, while the old table remained absent. A ₹100 partial payment submitted with the remarks property omitted returned success and persisted an empty remark. Temporary zero-GST and Advance variants both omitted the tracker, then the test record was restored. Full verification: `tsc --noEmit` clean, ESLint clean, Vitest **66/66 files, 444 passed, 7 intentionally skipped**, and production build passed (the first sandboxed Turbopack attempt was environment-blocked from binding its worker port; the unchanged escalated rerun passed).
- Current-main integration verification after merging the intervening vendor features: TypeScript and ESLint clean, Vitest **72/72 files, 469 passed, 7 intentionally skipped**, and the Next.js production build passed.
...
- The notification is invoked from `performAuthorityApproval()` only after the recommendation transaction succeeds. Both the authenticated Authority Dashboard route and the token-link route already converge on this shared action, so both now send the same email without duplicated route-level mail logic.
- Provider failures remain non-blocking and use the established `EMAIL_SEND_FAILED` audit behavior: a successful recommendation is not rolled back because an informational email fails. Preview mode remains the default unless `EMAIL_MODE=live`; production's configured live provider controls actual delivery.
- Verification: TypeScript and ESLint clean; focused route/notifier coverage confirms the real submitter address and MCCIA HTML content; full Vitest suite **60/60 files, 412 passed, 7 intentionally skipped**; production build passed. No real recommendation was performed during verification, so no production submission was mutated and no unsolicited live email was sent.
...
### Closed diagnosis — no submitter email previously existed at Authority Recommend step (Codex, 2026-09-09)
- The production incident was confirmed as a missing feature rather than a delivery regression: the recommendation action previously performed only the database update and audit write, with no mail call. The human subsequently approved adding the notification; the shipped entry immediately above records the resolution.
...
- 🔴 **Gemini Tax Invoice auto-fill needs `GEMINI_API_KEY` before live verification or deployment (2026-09-20).** The implementation is on `feature/gemini-invoice-autofill`; the secret is intentionally absent from `.env.local` and must be set only in local `.env.local` and Vercel project settings, never committed. After provision, run two explicit-click tests: a legible real NEFT invoice must populate editable Bill No./Date/Basic/GST and only a confident existing vendor; a deliberately unreadable invoice must leave the form manual with no error. Confirm the Gemini model/key quota is enabled for `gemini-2.5-flash` at that time.
...
```
2026-09-16 — Codex — Integrated the completed Arrears/TDS Payable Calculator
and GST Settlement Flag into current `main`. Renumbered the additive migration
from the stale branch's conflicting 0021 to `0023_payable_gst`, after the real
0021/0022 vendor-bank migrations. The human explicitly approved compatibility
initialization for the one existing partially paid advice; the migration copies
its already-saved Bill Passed For into `payable_amount` without changing the
bill amount, payment entries, or reference number. Regular Cash Voucher retains
the existing Bill Passed For flow; regular NEFT/Advance use the shared TDS
calculator, and Record a Payment remarks are optional. Applied 0023 successfully
and verified `MCCIA/2026-27/0001` as 5040.00 passed/payable and 4800.00 paid.
Current-main checks: tsc, ESLint, 469 tests (7 skipped), and production build pass.

2026-09-15 — Codex — Simplified the unmerged regular-NEFT GST Settlement
Tracker on `gst-settlement-payable-calculator-wip` to a direct Yes/No flag:
no settlement amount or remarks, with `gst_settled`, `gst_settled_by`, and
`gst_settled_at` stored on `payment_advices` and attributed from the Finance
session. Production has no `gst_settlements` table and the isolated dev table
had zero rows, so migration 0021 was regenerated without that table and no
settlement history was lost. Also made Record a Payment remarks optional in
the client and server while preserving the existing NOT NULL DB column via an
empty-string compatibility value. Isolated live tests confirmed the GST flag,
actor/time display, zero-GST/Advance hiding, and a no-remarks partial payment.
TypeScript, ESLint, 444 tests (7 skipped), and production build pass. Migration
0021 remains unapplied to production; the existing partial-payment compatibility
backfill still awaits explicit human confirmation.
...
Tax Invoice remains the only mandatory Payment Advice document.
...
2026-09-09 — Codex — With explicit human approval, added the missing MCCIA HTML submitter notification after a successful Authority recommendation. Both dashboard and token-link routes use the shared recommendation action and therefore send consistently; provider failure remains non-blocking and auditable. Subsequently live-tested through a real temporary Recommend action: Gmail accepted message `<7d09aaed-591d-44f3-9672-2430d5d5f6b8@gmail.com>` and the temporary advice/audit records were removed afterward without consuming a serial number. Shipped separately as `e5a2426`. TypeScript/ESLint clean, full Vitest suite and production build passed.

2026-09-09 — Codex — Implemented the permanent `REJECTED` terminal state on isolated branch `feature/rejected-submissions` (migration `0020_shiny_iron_lad.sql`; **not applied to production and not merged to main at this point**). Recommending Authorities can Reject from their authenticated detail page or token-link page; Finance roles can Reject from Finance detail; DG is explicitly read-only in UI and both Authority server routes. Required remarks, actor/time, audit entry, submitter HTML email, edit-token invalidation, retained reference number, separate fuchsia Rejected status/tab, active-queue exclusion, Finance detail/history visibility, and explicit Excel export fields are included. The shared transaction permits only open `SUBMITTED`/`SENT_BACK` records and atomically blocks rejection when `payment_done_at` exists, `total_paid` is non-zero, or any `payment_entries` row exists.

Verification used a fully isolated local PostgreSQL database with migrations 0000–0020. Transactional records representing Authority Reject and Finance Reject both closed correctly, invalidated their edit links, wrote audit history, and preserved their reference numbers; a third record with a payment entry produced `UPDATE 0` and remained `SUBMITTED`; the serial counter remained exactly 55. The transaction was rolled back, leaving no test records. ESLint passed, full Vitest passed (62 files, 420 passed, 7 intentionally skipped), and the production build passed. A local preview database contains only throwaway dev accounts/records for human UI review; production data was not used or changed by Reject testing.

2026-09-09 — Claude — Created Aarya Tayade's Team Dashboard login, per the
human's request ("give access to Aarya as well," alongside Aishwarya's
existing MSME HELPLINE access). Checked `staff_members` first — one clean,
active row on file (`aaryat@mcciapune.com`, "Aarya Tayade"); no prior
`admin_users` row existed for her, no duplicate created. New `admin_users`
row (`DEPARTMENT` role) plus one `admin_user_roles` row (`scope_value =
"MSME HELPLINE"`, same scope as Aishwary Songirkar), using the same
predictable-password convention as the 2026-09-04 Authority expansion batch
and Satish Joshi's account — lowercase first name + `@2026` → `aarya@2026`.
No forced password change, matching every other account created this way.
Live-tested: logged in as her at `/team/login`, confirmed Team Submissions
...
Merged into `main` on 2026-09-16 after the human explicitly requested that
all remaining local changes be committed and pushed (`ebc7da7`, documentation
follow-up `3a053b4`).
