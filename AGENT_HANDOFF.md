...
- 🟡 **Live-send verification: partially done.** The human supplied a real `RESEND_API_KEY` and set `EMAIL_MODE=live` + `EMAIL_TEST_OVERRIDE_RECIPIENT` in their local `.env.local` (2026-07-30). Claude Code restarted the dev server to pick up the new env vars and drove two real submissions against the live Resend API: (1) an authority-with-email-on-file case — both `notifySubmissionConfirmation` and `notifyAuthorityApproval` returned real Resend message IDs (not preview logs), and the override redirect worked correctly (`authority approval` log showed `redirected from neerajt@mcciapune.com` to the override address); (2) an authority-with-no-email-on-file case (DG) — `notifySubmissionConfirmation` sent live, `notifyAuthorityApproval` correctly logged `"No email on file for authority DG, falling back to preview."` and did **not** call Resend. Test rows + attachments cleaned up afterward. **Not yet confirmed by a human:** whether the emails actually landed in the override inbox, whether the MCCIA logo image rendered, and whether the `[TEST — would go to: ...]` subject prefix displayed correctly — Claude Code cannot see a real inbox. The human should check the override inbox directly and report back; once confirmed, mark 🟢 and this item can close. `EMAIL_MODE` in `.env.local` is currently `live` (local dev only) — production/Vercel env is untouched and still defaults to preview.
